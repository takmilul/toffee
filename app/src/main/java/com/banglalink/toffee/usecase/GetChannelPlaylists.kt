...
class GetChannelPlaylists(private val preference: Preference, private val toffeeApi: ToffeeApi): SingleListRepository<ChannelPlaylist> {
    
    var mOffset: Int = 0
        private set
    private val limit = 10
    
    override suspend fun execute(): List<ChannelPlaylist> {
        val response = tryIO {
        ...
        if (response.response.channels != null) {
            return response.response.channels.map {
                ...
                ChannelPlaylist(it.id, it.poster_url_mobile, it.view_count, it.program_name)
            }
        }
        return listOf()
    }
